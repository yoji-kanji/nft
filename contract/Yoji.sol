...
contract Yoji is Ownable, ReentrancyGuard {
    ...
    // ERC721Metadata
    string public name = 'Yoji Kanji';
    string public symbol = 'YOJI';
    string public baseURI = 'https://yoji.info/api/metadata/';
    ...
    uint256 private constant COMBINATIONS = SHIFT3 * NUM_CHARS;
    ...
    // Customized (public)
    function setTokenPrice(uint256 _price) external onlyOwner returns (bool) {
        TOKEN_PRICE = _price;
        return true;
    }
    ...
    function _buyNFT(uint256 _tokenId) external payable nonReentrant returns (bool) {
        require(msg.value >= TOKEN_PRICE, 'Customized: msg.value is less than TOKEN_PRICE');
        if(msg.value > TOKEN_PRICE) payable(msg.sender).transfer(msg.value - TOKEN_PRICE);
    ...
    function _checkTokenId(uint256 _tokenId) public view returns (address _owner, string memory _chars) {
        (uint256 char1, uint256 char2, uint256 char3, uint256 char4) = tokenIdToArray(_tokenId);
        _owner = _owners[_tokenId];
        _chars = string(abi.encodePacked(charAt(char1), charAt(char2), charAt(char3), charAt(char4)));
    }

    function permutate(uint256 _tokenId, uint256 _index) external returns (bool) {
        require(ownerOf(_tokenId) == msg.sender, 'Customized: caller is not owner of tokenId');

        uint256 permutatedTokenId = getPermutatedTokenId(_tokenId, _index);

        _mint(msg.sender, permutatedTokenId);
    ...
    function getPermutatedTokenId(uint256 _tokenId, uint256 _index) public pure returns (uint256) {
        uint256[] memory original = new uint256[](4);

        (original[0], original[1], original[2], original[3]) = tokenIdToArray(_tokenId);

        uint256 index1 = _index / 1000 - 1;
        uint256 index2 = _index / 100 % 10 - 1;
        uint256 index3 = _index / 10 % 10 - 1;
        uint256 index4 = _index % 10 - 1;

        return original[index1] * SHIFT3
             + original[index2] * SHIFT2
             + original[index3] * SHIFT1
             + original[index4];
    }
